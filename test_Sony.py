...
in_files = glob.glob(input_dir + "/*")
for k in range(len(in_files)):
    in_path = in_files[k]
    in_fn = os.path.split(in_path)[-2]
    print(in_fn)
    ...
    Image.fromarray((origin_full*255).astype('uint8')).save(result_dir + 'original_%s.jpg'%(in_fn))
    Image.fromarray((output*255).astype('uint8')).save(result_dir + '%s.jpg'%(in_fn))
    #Image.fromarray((scale_full*255).astype('uint8')).save(result_dir + 'scale_full.png'%(test_id,ratio))
